...
6. 區分「療程」與「帶回家藥物」：化療、電療（放射線）、物理治療等在院內進行的療程放 treatments；病人帶回家自行服用的口服/外用藥物放 medication。療程不要塞進 medication，也不需要 shape/color/timing。
7. 每個 medication 判斷服用時段 timing（早/中/晚/睡前，可多個）與 meal_relation（飯前/飯後/睡前/空腹）：例「一天三次飯後」→ timing ["早","中","晚"]、meal_relation "飯後"；「睡前一顆」→ timing ["睡前"]。
8. 行程科別 department 以「轉診單」為準：若文件含轉診單，回診科別要用轉診後的目的科別（例：轉中醫就填「中醫」），不可沿用原開單科別。多份文件衝突時，轉診單優先。
9. documents：每一張上傳的文件各列一筆，含 title（文件標題或類型）、date（文件上的日期，有才填）、summary（一句話摘要）。有幾張文件就列幾筆。
...

{
  ...
  "documents": [{"title":"文件標題或類型","date":"YYYY-MM-DD","summary":"一句話摘要"}],
  "treatments": [{"name":"","type":"化療|電療|物理治療|其他療程","schedule":"例：每3週一次共6次","frequency":"","location":"","notes":""}],
